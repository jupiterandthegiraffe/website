  ...
  serviceSlider.addEventListener('input', (e) => {
    ...
    Array.from(sliderInfoSlides).forEach((slide, index) => {
        ...
        if ((index + 1) === value) {
            ...
            const titleText = new SplitText(slide.querySelector('h3'), {type: 'chars'})
    ...
    const calculateHeight = () => {
        ...
        Array.from(sliderInfoSlides).forEach((slide) => {
            ...
            const currentSlideHeight = parseInt(getComputedStyle(slide).getPropertyValue('height'))
            totalHeight = currentSlideHeight > totalHeight ? currentSlideHeight : totalHeight
